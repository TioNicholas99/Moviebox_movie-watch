  ...
  for (let i = 0; i < users.length; i++) {
    if (users[i].email === email) {
      ...
      if (users[i].password === password) {
        ...
        let welcomeMessage = "Selamat datang, " + email + "! Anda telah berhasil login.";
        alert(welcomeMessage);
        // Atau menambahkan pesan ke dalam elemen di halaman HTML
        document.querySelector("#result").innerText = welcomeMessage;

        // Menyimpan informasi pengguna yang sudah login ke dalam local storage
